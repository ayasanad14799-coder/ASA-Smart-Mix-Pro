...
def send_to_google_form(data):
    ...
    payload = {
        ...
        "entry.1503898770": data['Sust'], "entry.1583578049": "ASA_FINAL_V9.0"
    }
    try:
...
with hm:
    st.markdown("<h1 class='main-title'>ASA Smart Mix Pro v9.0</h1>", unsafe_allow_html=True)
    st.markdown("<div class='thesis-title'>Multi-criteria analysis of eco-efficient concrete from Technical, Environmental and Economic aspects</div>", unsafe_allow_html=True)

# استعادة بيانات الباحثة والمشرفين
...
if st.session_state.calc_results:
    ...
    with t1:
        ...
        if st.button("📤 Sync Report to Cloud"):
            data = {"Cement": cem, "Water": wat, "NCA": nca, "NFA": nfa, "RCA_P": rca, "MRCA_P": mrca, "SF": sf, "FA": fa, "Fiber": fib, "W_C": wc, "SP": sp, "CS_28": round(res['28d'], 2), "STS": round(res['sts'], 2), "EM": round(res['em'], 2), "CO2": round(res['co2'], 2), "Cost": round(res['cost'], 2), "Sust": round(res['sust'], 5), "Rank": "A"}
            if send_to_google_form(data): st.balloons(); st.success("✅ Recorded!")
    ...
    with t3:
        st.subheader("Technical Validation & Domain")
        v1, v2 = st.columns(2)
...
with t5:
    ...
    st.markdown(f"""<div class='doc-card'><b>Algorithm:</b> Multi-output Random Forest Regression<br><b>Database:</b> DIAMOND Meta-Data Repository ({len(db)} samples)<br><b>Reliability:</b> COV ≈ 6.16%</div>""", unsafe_allow_html=True)
    with st.form("Feedback_Form"):
        st.write("Submit Laboratory Observations:")
        lab_val = st.number_input("Lab Result (MPa)")
        if st.form_submit_button("Submit Data"): st.success("Feedback Logged Successfully!")

# 7. الفوتر (استعادة إخلاء المسؤولية الرسمي)
st.markdown(f"""
